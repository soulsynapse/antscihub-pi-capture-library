...
8. Read loop interval from `<desktop>/4-CAPTURE/config/recording-loop.txt` (or `ANTCAM_LOOP_VALUE_FILE` override, defaults to `1m`).
...
- `video.sh`
  ...
  - Uses configurable length (`antcam set length <duration>`, default `0s`), segment size (`antcam set segment <duration>`, default `1m`), and loop interval (`antcam set loop <duration>`, default `1m`)
...
- `photos.sh`
  ...
  - Uses configurable length (`antcam set length <duration>`, default `0s`) and loop interval (`antcam set loop <duration>`, default `1m`)
